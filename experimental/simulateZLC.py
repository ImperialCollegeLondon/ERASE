...
def simulateZLC(**kwargs):
    ...
    if modelType == 'Diffusion' or modelType == 'Diffusion1T':
        ...
        inputParameters = (adsorbentDensity, isothermModel, rateConstant_1, rateConstant_2, rateConstant_3,
                           flowIn, feedMoleFrac, initMoleFrac, pressureTotal, 
                           temperature, volSorbent, volGas, modelType, rpore, Dpvals, numPellets)
        tspan, Y, r, yOut, flowOut, qAverage = DiffusionAdsorption1Dtau(initMoleFrac, t_eval, volSorbent, volGas, adsorbentDensity, particleEpsilon, flowIn, temperature, pressureTotal, [isothermModel], rateConstant_1, rateConstant_2, rateConstant_3, rpore, Dpvals, numPellets)
        # Presure vector in output
        pressureVec =  pressureTotal * np.ones(len(tspan)) # Constant pressure
        ...
        # pdb.set_trace()
        # Parse out the output matrix and add flow rate
        resultMat = np.row_stack((yOut,qAverage,pressureVec,flowOut,np.transpose(Y)))
        # pdb.set_trace()
        # Parse out the time
        timeSim = tspan
        ...
        yOut = np.transpose(yOut)
        flowOut = np.transpose(flowOut)
        tempVals = np.transpose(tempVals)
        # Parse out the output matrix and add flow rate
        resultMat = np.row_stack((yOut,qAverage,pressureVec,flowOut,tempVals))
        # pdb.set_trace()
        # Parse out the time
        timeSim = tspan
    elif modelType == 'DV':
...
def radialDiffusionAdsorption1D(x, t, r, n, isothermModel, temperature, rateConstant_1, rateConstant_2, rateConstant_3, epsilon, adsorbentDensity,volGas, volSorbent, volFlow, Y0, Rp):
    ...
    volMix = 0.785e-6
    DnDt = (volSorbent*3 / ( (Rp)**3 ) * np.trapz(DqDt[0:n-3] * r[0:-3] ** 2, r[0:-3])+
            volGas*3 /     ( (Rp)**3 ) * np.trapz(DcDt[0:n-3] * r[0:-3] ** 2, r[0:-3]))/(volSorbent+volGas)
...
def DiffusionAdsorption1Dtau(Y0, tspan, volSorbent, volGas, adsorbentDensity, epsilon, volFlow, temperature, Ptotal, isothermModelAll, rateConstant_1, rateConstant_2, rateConstant_3, rpore, Dpvals, numPellets):
    ...
    for isothermModel in isothermModelAll:
        ...
        q0[-2:] = 0
        x0 = np.concatenate([c0, q0])    
        inputArgs = r, n, isothermModel, temperature, rateConstant_1, rateConstant_2, rateConstant_3, epsilon, adsorbentDensity,volGas, volSorbent, volFlow, Y0, Rp, rpore, Dpvals, numPellets
        ...
        qAverage_i = np.zeros(len(tspan))
        for jj in range(len(tspan)):
            qAverage_i[jj] = 3 / (r[-3]**3) *             np.trapz(Y[jj, n:2*n-3] * r[0:-3] ** 2, r[0:-3])
            moleGas[jj] = volGas * 3 / (r[-3]**3) *       np.trapz(Y[jj, 0:n-3]   * r[0:-3] ** 2, r[0:-3])
            moleSolid[jj] = volSorbent * 3 / (r[-3]**3) * np.trapz(Y[jj, n:2*n-3] * r[0:-3] ** 2, r[0:-3])
        ...
        print("Mass Mass Balance Error",str(round(MBError,3)),"%")
        # pdb.set_trace()
...
def radialDiffusionAdsorption1Dtau(x, t, r, n, isothermModel, temperature, rateConstant_1, rateConstant_2, rateConstant_3, epsilon, adsorbentDensity,volGas, volSorbent, volFlow, Y0, Rp, rpore, Dpvals, numPellets):
    ...
    DyDt = 1/(volMix) * ((volFlow*0 - flowOut*c[-2]*(Rg*temperature)/Ptotal) - (numPellets*(volSorbent+volGas)*(Rg*temperature)/Ptotal)*DnDt)
...
def DiffusionAdsorption1DNItau(Y0, tspan, volSorbent, volGas, adsorbentDensity, epsilon, volFlow, temperature, Ptotal, isothermModelAll, rateConstant_1, rateConstant_2, rateConstant_3, rpore,numPellets,Dp):
    ...
    for isothermModel in isothermModelAll:
        ...
        tempVals = tempVals[:,-1]
        for jj in range(len(tspan)):
            qAverage_i[jj] = 3 / (Rp**3) *             np.trapz(Y[jj, n:2*n-3] * r[0:-3] ** 2, r[0:-3])
            moleGas[jj] = volGas * 3 / (Rp**3) *       np.trapz(Y[jj, 0:n-3]   * r[0:-3] ** 2, r[0:-3])
            moleSolid[jj] = volSorbent * 3 / (Rp**3) * np.trapz(Y[jj, n:2*n-3] * r[0:-3] ** 2, r[0:-3])
        moleTotal = moleSolid + moleGas
        moleRate = np.gradient(moleTotal, tspan)
        volRate = -moleRate * Rg * tempVals / Ptotal
        yOut = Y[:,n-2]*(Rg*tempVals)/Ptotal
        volFlowOut = (volRate + volFlow)
        qAverage = qAverage_i
        yOut[yOut < 1e-5] = 1e-5
        volFlowOut[volFlowOut < volFlow] = volFlow
...
def radialDiffusionAdsorption1DNItau(x, t, r, n, isothermModel, temperature, rateConstant_1, rateConstant_2, rateConstant_3, epsilon, adsorbentDensity,volGas, volSorbent, volFlow, Y0, Rp, rpore,numPellets,Dpvals):
    ...
    DQDt = (volSorbent+volGas)*(3 / ( (Rp)**3 ) * np.trapz(DqDt[0:n-3] * r[0:-3] ** 2, r[0:-3]))
    # DQDt = (volSorbent+volGas)* 3 /(Rp**3) * np.trapz(DqDt[0:n-3] * r[0:-3] ** 2, r[0:-3])
    # pdb.set_trace()
    DcDt[-2] = DyDt*Ptotal/(Rg*temperature)
    DcDt[-1] = 0
    DqDt[-2:] = 0
    DTDt[:] = 1/(volSorbent*adsorbentDensity*Cs) *(DQDt*np.mean(heatAds)+(ha)*(temperature-T[0]))
    # DTDt[:] = 1/(volSorbent*adsorbentDensity*Cs) *(DQDt*np.mean(heatAds)+ha*(temperature-T[0]))
...
def ChapmanEnskog(kTbyeps12,temperature,MwCar,sigma12):
    import pandas as pd
    import os
    import numpy as np
    data = np.array(pd.read_csv(os.path.join('..','ERASE','experimental','ChapmanEnskogVals.csv')))
    MwCO2 = 44.01
    sigma12
    omegaD = np.interp(kTbyeps12,data[:,0],data[:,1])
    Ptotal = 1e5
    DmVal = (0.001858*temperature**1.5 *(1/MwCO2 + 1/MwCar)**0.5) /(Ptotal*(sigma12*1e10)**2*omegaD)*1e-4
    return DmVal
