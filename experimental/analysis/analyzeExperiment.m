...
if flagCalibration
    ...
    if experimentStruct.initialCO2 % if initial gas is CO2
        ...
        moleFracThresholdInd = min([find(outputStruct.moleFrac(:,1)<experimentStruct.moleFracThreshold,1,'first'),...
            find(~isnan(totalFlowRate),1,'last')]); % Additional check to weed out nan flow due to interpolation
        % Set the final index to be the length of the series, if threshold not
        % reached
        if isempty(moleFracThresholdInd)
            moleFracThresholdInd = length(outputStruct.moleFrac(:,1));
        end
    end
    ...
    if experimentStruct.initialCO2 % if initial gas is CO2
        ...
        experimentOutput.moleFrac = outputStruct.moleFrac(1:moleFracThresholdInd,1); % Mole fraction He [-]
    end
