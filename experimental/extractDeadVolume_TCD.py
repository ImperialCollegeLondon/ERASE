...
def extractDeadVolume_TCD(**kwargs):
    import numpy as np
    ...
    if 'fileName' in kwargs:
        ...
        fileName = ['ZLC_Empty_Exp04_CO2_N2_des_TCD_05_60ccm_2_Output.mat']
    ...
    # When the actual dead volume is used, diffusive volume allowed to change 
     ...
     #       optBounds = np.array(([np.finfo(float).eps,20], [np.finfo(float).eps,20],
     #                             [np.finfo(float).eps,20], [1,50], [np.finfo(float).eps,0.1]))
            optBounds = np.array(([np.finfo(float).eps,4], [np.finfo(float).eps,50],
                                  [np.finfo(float).eps,50], [1,50], [np.finfo(float).eps,0.2],
                                  [np.finfo(float).eps,50], [np.finfo(float).eps,50],
                                                        [np.finfo(float).eps,0.2],
                                  [np.finfo(float).eps,50], [np.finfo(float).eps,50],
...
def deadVolObjectiveFunction(x):
    ...
    for ii in range(len(filePath)):
        ...
        flowRateTemp = load(filePath[ii])["volFlowRate"].flatten()
        ...
        # pdb.set_trace()
        moleFracSimALL = np.hstack((moleFracSimALL, (np.transpose(moleFracSim)-minExp)/normalizeFactor))
